package com.pixilaregames.thrivecraft.blocks.tileentity;
...
public class TileEntityPetrifiedChest extends TileEntityLockableLoot implements ITickable
{
	...
	{
		return this.hasCustomName() ? this.customName : "container.petrified_chest";
	}
	...
	{
		return new ContainerPetrifiedChest(playerInventory, this, playerIn);
	}
	
	@Override
	public String getGuiID() 
	{
		return Reference.MOD_ID + ":petrified_chest";
	}
	...
	public void update()
	{
		if (!this.world.isRemote && this.numPlayersUsing != 0 && (this.ticksSinceSync + pos.getX() + pos.getY() + pos.getZ()) % 200 == 0)
        {
            ...
            {
                if (entityplayer.openContainer instanceof ContainerPetrifiedChest)
                {
                    if (((ContainerPetrifiedChest)entityplayer.openContainer).getChestInventory() == this)
                    {
