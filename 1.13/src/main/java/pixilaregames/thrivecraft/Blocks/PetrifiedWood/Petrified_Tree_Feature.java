...
import pixilaregames.thrivecraft.Blocks.ThriveCraftBlockTags;
...
{
	private static final IBlockState DEFAULT_TRUNK = BlockList.petrified_log.getDefaultState();
	private static final IBlockState DEFAULT_LEAF = BlockList.petrified_leaves.getDefaultState();
	...
	protected net.minecraftforge.common.IPlantable sapling = (net.minecraftforge.common.IPlantable)BlockList.petrified_sapling;

	public Petrified_Tree_Feature(boolean p_i2027_1_)
	{
		this(p_i2027_1_, 4, DEFAULT_TRUNK, DEFAULT_LEAF, false);
	}

	public Petrified_Tree_Feature(boolean notify, int minTreeHeightIn, IBlockState woodMeta, IBlockState p_i46446_4_, boolean growVines)
	{
		super(notify);
	...
	public boolean place(Set<BlockPos> changedBlocks, IWorld worldIn, Random rand, BlockPos position)
	{
	    ...
	    if (position.getY() >= 1 && position.getY() + i + 1 <= worldIn.getWorld().getHeight())
	    {
	    	for(int j = position.getY(); j <= position.getY() + 1 + i; ++j)
	        {
	         ...
	         {
	        	 if (worldIn.getBlockState(position.down()).canSustainPlant(worldIn, position.down(), net.minecraft.util.EnumFacing.UP,  (pixilaregames.thrivecraft.Blocks.PetrifiedWood.Petrified_Sapling)BlockList.petrified_sapling) && position.getY() < worldIn.getWorld().getHeight() - i - 1)
	        	 {
	        			 ...
	        			 for(int k3 = position.getY() - 3 + i; k3 <= position.getY() + i; ++k3)
	        			 {
	        				 ...
	        				 for(int l4 = position.getX() - k4; l4 <= position.getX() + k4; ++l4)
	        				 {
	        					 for(int i5 = position.getZ() - k4; i5 <= position.getZ() + k4; ++i5)
	        					 {
	        						 ...
	        						 if (worldIn.getBlockState(blockpos$mutableblockpos1).isIn(ThriveCraftBlockTags.LEAVES))
	        						 {
	   	...
	   	public Petrified_Tree_Feature setSapling(net.minecraftforge.common.IPlantable sapling)
	   	{
